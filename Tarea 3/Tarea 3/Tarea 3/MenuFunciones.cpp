...
void Menu::print_CalificacionesCallback(string Nombre, string Apellido, int Edad, vector <Estudiante>* Estudiantes, void (Menu:: *callback)(int, vector<Estudiante>*)) {
	int POS = 0;
	POS = encontrarEstudiante(Nombre, Apellido, Edad, Estudiantes);
	if (POS == -1) {
		cout << "No existe tal Estudiante" << endl;
...
void Menu::ingresar_CalificacionCallback(string Nombre, string Apellido, int Edad, string Materia, int Nota, vector<Estudiante>* Estudiantes, void (Menu:: *callback)(string, int, int, vector<Estudiante>*)) {
	if (Nota < 0) {
		cout << "Nota no puede ser menor a 0" << endl;
		Separacion();
		return;
...
Menu::Menu() {
	...
	while (true) {
		...
		cout << "2 - Obtener Calificaciones de un Estudiante" << endl; // Callback en esta Opción
		cout << "3 - Ingresar Estudiante" << endl;
		cout << "4 - Borrar un Estudiante" << endl;
		cout << "5 - Ingresar Calificacion a un Estudiante" << endl; // Callback en esta Opción
		...
		case 2:
			if (Estudiantes->empty() == true) {
				...
				print_CalificacionesCallback(NOM, APE, EDA, Estudiantes, &Menu::callbackEstudiantes);

				// print_Calificaciones(NOM, APE, EDA, Estudiantes); // Implementación Vieja
			}
			break;
		...
		case 5:
			...
			ingresar_CalificacionCallback(NOM, APE, EDA, MAT, NOT, Estudiantes, &Menu::callbackProfesores);
			
			// ingresar_Calificacion(NOM, APE, EDA, MAT, NOT, Estudiantes); // Implementación Vieja
			break;
		...
		case 6:
			...
			NOT = obtenerNota();
			Separacion();
			eliminar_Calificacion(NOM, APE, EDA, MAT, NOT, Estudiantes);
			break;
